...
# load shape
object_urdf_fn = '../asset/original_sapien_dataset/dataset/%s/mobility.urdf' % shape_id
...
depth_min = depth.min()
depth_max = depth.max()
depth_normalized = (depth - depth_min) / (depth_max - depth_min)  # Normalize to [0, 1]
depth_8bit = (depth_normalized * 255).astype(np.uint8)  # Convert to 8-bit grayscale

 # Save depth map
Image.fromarray(depth_8bit).save(os.path.join(out_dir, 'original_depth.png'))
