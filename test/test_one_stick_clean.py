...
parser.add_argument('--overwrite', action='store_true', default=False,
                    help='overwrite if out_dir exists [default: False]')
...
# print("eval_conf.record_name:",eval_conf.record_name)
...
if abs(d_x) > 1 or abs(d_y) > 1 or abs(d_z) > 1:  # If d_x, d_y, d_z are not normalized
    gripper_direction_camera = np.array([d_x * 0.02, d_y * 0.02, d_z * 0.02])
else:
    gripper_direction_camera = np.array([d_x, d_y, d_z])

if abs(fd_x) > 1 or abs(fd_y) > 1 or abs(fd_z) > 1:
    gripper_forward_direction_camera = np.array([fd_x * 0.02, fd_y * 0.02, fd_z * 0.02])
else:
    gripper_forward_direction_camera = np.array([fd_x, fd_y, fd_z])
...
# print("cam_XYZA:", cam_XYZA.shape)
position_cam = cam_XYZA[x, y, :3]

position_cam_xyz1 = np.ones((4), dtype=np.float32)
position_cam_xyz1[:3] = position_cam
# print("position_cam_xyz1:", position_cam_xyz1)
...
def plot_mani(cam, up, forward):
    ...
    # print("out_info['gripper_direction_world']:", out_info['gripper_direction_world'])
    ...
    # print("final_rotmat:", final_rotmat)
    ...
    # print("position_world_xyz1_end:", position_world_xyz1_end)
    ...
    if success == True:
        ...
        # print("final_target_part_qpos:", out_info['final_target_part_qpos'], "start_target_part_qpos:",
        #       out_info['start_target_part_qpos'])
...
print("success:", success, "mani_succ:", mani_succ)
