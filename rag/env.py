...
import numpy as np
try:
    from rag.utils import process_angle_limit, get_random_number
except ImportError:
...
class Env(object):
    ...
    def quaternion_to_rotation_matrix(self, quaternion):
        ...
        Converts a quaternion representation to a 3x3 rotation matrix using
        scipy's spatial transformation utilities.
        
        Args:
            quaternion (np.ndarray): Quaternion as [x, y, z, w] array
    ...
    def rotation_matrix_to_euler_angles(self, rotation_matrix):
        ...
        Converts a 3x3 rotation matrix to Euler angles using XYZ convention.
        The rotation order can be modified based on specific requirements.
        
        Args:
            rotation_matrix (np.ndarray): 3x3 rotation matrix
            
        Returns:
            np.ndarray: Euler angles in radians (roll, pitch, yaw)
        """
        r = R.from_matrix(rotation_matrix)
        euler_angles = r.as_euler('xyz')  # Or use other rotation orders like 'zyx', depending on your needs
        return euler_angles
    def compute_inverse_kinematics(self, delta_pose_world, ee_jacobian, damping=0.05):
        ...
        Calculates joint angle changes required to achieve a desired end-effector
        pose change using the Jacobian matrix and damped least squares regularization
        to avoid singularities.
        
        Args:
            delta_pose_world (np.ndarray): Desired change in end-effector pose (6-DOF)
            ee_jacobian (np.ndarray): End-effector Jacobian matrix (6 x n_joints)
            damping (float): Damping factor for regularization (default: 0.05)
            
        Returns:
            np.ndarray: Joint angle changes required to achieve desired pose change
        """
    ...
    def move_robot(self, env, robot: sapien.Articulation, start_pose: np.ndarray, ee_link: np.ndarray, num_steps, cam):
        """
        Move robot end-effector to target pose through incremental steps.
        
        Calculates and executes robot motion to move the end-effector from its current
        position to the target pose by breaking the motion into incremental steps.
        Uses rotation matrix calculations for accurate orientation control.
        
        Args:
            env: Environment object containing the simulation
            robot (sapien.Articulation): Robot articulation object
            start_pose (np.ndarray): Target pose for the end-effector
            ee_link (np.ndarray): Current end-effector link
            num_steps (int): Number of incremental steps for the motion
            cam: Camera object for visual feedback
        """
    ...
    def set_target_object_part_actor_id(self, actor_id):
        ...
        return action_type,hinge_pose,joint
